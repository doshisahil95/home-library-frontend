...
// All sub-components defined outside Dashboard so their identity is stable
// across renders — prevents unnecessary unmount/remount cycles
...
function BarRow({ label, count, max }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="w-32 text-sm text-gray-600 dark:text-gray-300 truncate shrink-0">
        {label}
      </span>
      ...
      <span className="w-8 text-sm text-right text-gray-500 dark:text-gray-400 shrink-0">
        {count}
      </span>
    </div>
  );
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {stats.recentBooks.map((book) => (
                    <tr key={book._id} className="text-sm">
                      <td className="py-3 text-gray-800 dark:text-gray-100 truncate max-w-0 w-2/5 pr-4">
                        {book.title}
                      </td>
                      <td className="py-3 text-gray-600 dark:text-gray-300 truncate">
                        {book.author}
                      </td>
                      <td className="py-3 text-gray-600 dark:text-gray-300 truncate">
                        {book.house}
